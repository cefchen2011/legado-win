...
class AppServer(private val port: Int) {
    ...
    private fun handle(ex: HttpExchange) {
        ...
        try {
            when {
                ...
                path.startsWith("/mock/") || path.startsWith("/mock2/") -> serveMock(ex, path)
                path.startsWith("/dav/") -> serveMockDav(ex, path)
    ...
    private fun handleApi(ex: HttpExchange, api: String) {
        ...
        when (api) {
            ...
            /**
             * 第二个自测书源，与 /mock 提供同一本书。
             * 用途：验证「换源」能在另一个源里找到同一本书。
             */
            "mock-source2" -> {
                val base = "http://127.0.0.1:$port/mock2"
                val text = """
                {
                  "bookSourceUrl": "$base",
                  "bookSourceName": "本地自测书源2",
                  "bookSourceGroup": "自测",
                  "enabled": true,
                  ...
                  "ruleBookInfo": {
                    ...
                    "author": "class.bookauthor@text",
                    "intro": "class.intro@text",
                    "tocUrl": "class.tocurl@href"
                  },
                  "ruleToc": {
                ...
                if (q["import"] == "1") {
                    ...
                    json(ex, 200, mapOf("ok" to true, "imported" to n, "source" to text))
                } else {
                    json(ex, 200, mapOf("ok" to true, "source" to text))
                }
            }

            /** 返回内置自测订阅源，并可直接带 ?import=1 导入 */
            "mock-rss-source" -> {                val base = "http://127.0.0.1:$port/mock"
                val text = """
                {
                  "sourceUrl": "$base/rss",
                  "sourceName": "本地自测订阅源",
            ...
            /**
             * 漫画自测书源：bookSourceType = 2（图片）。
             * 与文字书源唯一的区别就是这个类型字段——引擎据此走图片规则。
             */
            "mock-manga-source" -> {
                val base = "http://127.0.0.1:$port/mock/manga"
                val text = """
                {
                  "bookSourceUrl": "$base",
                  "bookSourceName": "本地自测漫画源",
                  "bookSourceGroup": "自测",
                  "bookSourceType": 2,
                  "enabled": true,
                  "searchUrl": "$base/search?key={{key}}",
            ...
            /**
             * 视频自测书源：bookSourceType = 4（视频）。
             * 章节页里放 video 标签，验证"从 HTML 抽播放地址"的兜底分支。
             */
            "mock-video-source" -> {
                val base = "http://127.0.0.1:$port/mock/video"
                val text = """
                {
                  "bookSourceUrl": "$base",
                  "bookSourceName": "本地自测视频源",
                  "bookSourceGroup": "自测",
                  "bookSourceType": 4,
                  "enabled": true,
                  "searchUrl": "$base/search?key={{key}}",
            ...
            /** 自测词典：验证词典规则（urlRule + showRule） */
            "mock-dict" -> {
                val base = "http://127.0.0.1:$port/mock/dict?key={{key}}"
                val rules = listOf(
                    mapOf(
                        "name" to "自测词典",
                        "urlRule" to base,
                        "showRule" to "id.result@html",
                        "enabled" to true,
                        "sortNumber" to 1,
                    ),
                    mapOf(
                ...
                if (q["import"] == "1") {
                    rules.forEach { m ->
                        appDb.dictRuleDao.upsert(
                            DictRule(
                                name = m["name"] as String,
                                urlRule = m["urlRule"] as String,
                                showRule = m["showRule"] as String,
                                enabled = true,
                                sortNumber = m["sortNumber"] as Int,
                            )
                        )
                    }
                    json(ex, 200, mapOf("ok" to true, "imported" to rules.size))
                } else {
                    json(ex, 200, mapOf("ok" to true, "rules" to rules))
                }
            }
            ...
            /**
             * 音频自测书源：bookSourceType = 1（音频）。
             * legado 四种书源类型（文字/音频/图片/视频）中最后一种。
             */
            "mock-audio-source" -> {
                val base = "http://127.0.0.1:$port/mock/audio"
                val text = """
                {
                  "bookSourceUrl": "$base",
                  "bookSourceName": "本地自测音频源",
                  "bookSourceGroup": "自测",
                  "bookSourceType": 1,
                  "enabled": true,
                  "searchUrl": "$base/search?key={{key}}",
            ...
            /** 返回内置自测书源，并可直接带 ?import=1 导入 */
            "mock-source" -> {
                val text = mockBookSourceJson()
                if (q["import"] == "1") {
                    val n = importSources("[$text]")
                    json(ex, 200, mapOf("ok" to true, "imported" to n, "source" to text))
                } else {
    ...
    /** 内置 mock WebDAV 的存储（仅用于自包含验证） */
    private val mockDavFiles = java.util.concurrent.ConcurrentHashMap<String, ByteArray>()
    private val mockDavDirs = java.util.concurrent.ConcurrentHashMap.newKeySet<String>()
    ...
    /**
     * 内置的最小 WebDAV 服务，只为自包含验证「备份到 WebDAV / 从 WebDAV 恢复」。
     * 支持 MKCOL（建目录）、PUT（存文件）、GET（取文件）。
     * 不做 PROPFIND / LOCK —— legado 的同步也只用到这三个动作。
     */
    private fun serveMockDav(ex: HttpExchange, path: String) {
        val key = path.removePrefix("/dav/")
        when (ex.requestMethod.uppercase()) {
            "MKCOL" -> {
                mockDavDirs.add(key.trimEnd('/'))
                ex.sendResponseHeaders(201, -1)
            }

            "PUT" -> {
                val bytes = ex.requestBody.readBytes()
                mockDavFiles[key] = bytes
                LogUtils.i("MockDav", "PUT $key (${bytes.size} bytes)")
                ex.sendResponseHeaders(201, -1)
            }

            "GET" -> {
                val bytes = mockDavFiles[key]
                if (bytes == null) {
    ...
    /**
     * 返回字节内容并支持 HTTP Range。
     *
     * 浏览器的媒体栈（`<audio>` / `<video>`）依赖 Range 请求来起播与拖动进度；
     * 不支持 Range 时元素会停在 `NETWORK_NO_SOURCE` 而不播放——
     * 图片/文本不受影响，所以这个问题只在音视频上暴露。
     */
    private fun serveBytesWithRange(ex: HttpExchange, bytes: ByteArray, contentType: String) {
        ...
        if (range != null && range.startsWith("bytes=")) {
            ...
            val end = parts.getOrNull(1)?.trim()?.toIntOrNull()?.coerceAtMost(total - 1)
                ?: (total - 1)
            if (start in 0 until total && end >= start) {
                ...
                ex.responseBody.use { it.write(bytes, start, len) }
                return
            }
    ...
    /**
     * 生成一段可播放的 WAV（16bit 单声道 PCM，约 2 秒正弦波）。
     * 用于音频书源自测——WAV 头很简单，不必依赖任何编码器。
     */
    private fun mockWav(freqHz: Double): ByteArray {
        ...
        for (i in 0 until total) {
            // 加淡入淡出，避免爆音
            val env = minOf(1.0, i / 800.0, (total - i) / 800.0)
            val v = (Math.sin(2 * Math.PI * freqHz * i / sampleRate) * 12000 * env).toInt()
    ...
    /** 生成一张带页号的测试 PNG，用于漫画阅读链路验证 */
    private fun mockPng(chapter: Int, page: Int): ByteArray {
        val w = 360
        val h = 520
        ...
        try {
            // 每话换个底色，方便肉眼确认翻话生效
            val palette = arrayOf(
                java.awt.Color(0x3F51B5), java.awt.Color(0x8B5E34), java.awt.Color(0x2F9E44)
            )
            g.color = palette[chapter % palette.size]
            g.fillRect(0, 0, w, h)
            g.color = java.awt.Color.WHITE
            g.font = java.awt.Font("SansSerif", java.awt.Font.BOLD, 42)
            val text = "第${chapter + 1}话"
            val sub = "第 ${page + 1} 页"
            g.drawString(text, (w - g.fontMetrics.stringWidth(text)) / 2, h / 2 - 30)
            g.font = java.awt.Font("SansSerif", java.awt.Font.PLAIN, 26)
            g.drawString(sub, (w - g.fontMetrics.stringWidth(sub)) / 2, h / 2 + 30)
        } finally {
            g.dispose()
        }
        val out = java.io.ByteArrayOutputStream()
    ...
    // ------------------------------------------------------------ 内置自测书源站点
    //
    // 用途：让"搜索 → 详情 → 目录 → 正文"整条链路可以在**不依赖外部网站**的情况下
    // 自证可用。配套的自测书源见 /api/mock-source。
    private fun serveMock(ex: HttpExchange, path: String) {
        val q = query(ex)
        // /mock/ 与 /mock2/ 提供同一本书：用于验证"换源"能找到另一个源
        val p = if (path.startsWith("/mock2/")) "/mock/" + path.removePrefix("/mock2/") else path
        val slot = if (path.startsWith("/mock2/")) "2" else "1"
        val html = when (p) {
            "/mock/search" -> {
                val key = q["key"] ?: ""
                """
                <html><body>
                  <div class="result">
                    <a class="name" href="/mock/book">测试书</a>
                    <span class="author">测试作者</span>
                    <span class="kind">玄幻</span>
                    <span class="src">源$slot</span>
                  </div>
                  <div class="result">
                    <a class="name" href="/mock/book2">$key 第二本</a>
            ...
            "/mock/book" -> """
                <html><body>
                  <h1 class="bookname">测试书</h1>
                  <span class="bookauthor">测试作者</span>
                  <div class="intro">这是一本用于验证 legado Windows 移植链路的测试书。</div>
                  <a class="tocurl" href="/mock/toc">目录</a>
                </body></html>
            """.trimIndent()

            "/mock/toc" -> """
                <html><body><div id="toc">
                  <a class="chapter" href="/mock/content?c=0">第一章 百世书</a>
                  <a class="chapter" href="/mock/content?c=1">第二章 顺天易</a>
                  <a class="chapter" href="/mock/content?c=2">第三章 魔门人材</a>
                </div></body></html>
            """.trimIndent()

            "/mock/content" -> {
                val c = q["c"]?.toIntOrNull() ?: 0
                """
                <html><body>
                  <div id="content">
                    <p>这是第 ${c + 1} 章的正文内容。</p>
                    <p>它由内置的 mock 站点提供，用于验证规则引擎与抓取链路。</p>
                    <p>段落 ${c + 1}-3：legado 的书源规则在这里被真实执行。</p>
                  </div>
                  <a class="next" href="/mock/content?c=${c + 1}">下一章</a>
                </body></html>
                """.trimIndent()
            }
            ...
            "/mock/rss" -> """
                ...
                <rss version="2.0">
                  <channel>
                    <title>本地自测订阅源</title>
                    <link>http://127.0.0.1:$port/mock/rss</link>
                    <description>用于验证 legado Windows 移植的订阅源链路</description>
                    <item>
                      <title>第一篇：移植进展</title>
                      <link>http://127.0.0.1:$port/mock/article?i=1</link>
                      <pubDate>Wed, 17 Sep 2026 10:00:00 GMT</pubDate>
                      <description>规则引擎、书源 JS API 面、抓取链路均已跑通。</description>
                    </item>
                    <item>
                      <title>第二篇：本地书解析</title>
                      <link>http://127.0.0.1:$port/mock/article?i=2</link>
                      <pubDate>Thu, 18 Sep 2026 10:00:00 GMT</pubDate>
                      <description>TXT 与 EPUB 的分章都复用了上游实现。</description>
                    </item>
                    <item>
                      <title>第三篇：朗读与订阅</title>
                      <link>http://127.0.0.1:$port/mock/article?i=3</link>
                      <pubDate>Fri, 19 Sep 2026 10:00:00 GMT</pubDate>
                      <description>朗读用浏览器语音，订阅源走上游 Rss 实现。</description>
                    </item>
                  </channel>
                </rss>
            """.trimIndent()
            ...
            "/mock/article" -> {
                ...
                <html><body>
                  ...
                  <div id="content">
                    <p>这是第 $i 篇文章的正文内容。</p>
                    <p>它由内置的 mock 订阅源提供，用于验证订阅源的抓取与阅读链路。</p>
                  </div>
                </body></html>
                """.trimIndent()
            }
            ...
            "/mock/manga/page" -> {
                val c = q["c"]?.toIntOrNull() ?: 0
                val pages = 3
                val imgs = (0 until pages).joinToString("\n") {
                    """  <img src="/mock/manga/img?c=$c&p=$it">"""
                }
                """
            ...
            "/mock/manga/img" -> {
                // 真正返回一张 PNG（带页号），用于验证漫画阅读链路
                val c = q["c"]?.toIntOrNull() ?: 0
                val p = q["p"]?.toIntOrNull() ?: 0
                val bytes = mockPng(c, p)
            ...
            "/mock/manga/search" -> """
                <html><body>
                  <div class="result">
                    <a class="name" href="/mock/manga/book">测试漫画</a>
                    <span class="author">漫画作者</span>
                    <span class="kind">热血</span>
                  </div>
                </body></html>
            """.trimIndent()
            ...
            "/mock/manga/book" -> """
                <html><body>
                  <h1 class="bookname">测试漫画</h1>
                  <span class="bookauthor">漫画作者</span>
                  <div class="intro">用于验证 legado Windows 移植的漫画阅读链路。</div>
                  <a class="tocurl" href="/mock/manga/toc">目录</a>
                </body></html>
            """.trimIndent()

            "/mock/manga/toc" -> """
                <html><body><div id="toc">
                  <a class="chapter" href="/mock/manga/page?c=0">第1话 起始</a>
                  <a class="chapter" href="/mock/manga/page?c=1">第2话 交锋</a>
                  <a class="chapter" href="/mock/manga/page?c=2">第3话 决战</a>
                </div></body></html>
            """.trimIndent()

            "/mock/dict" -> {
                // 极简词典接口：返回 HTML 片段，交给 showRule 提取
                val key = q["key"] ?: ""
                """
                <html><body>
                  <div id="result">
                    <p><b>$key</b>：这是自测词典对「$key」的解释。</p>
                    <p>用于验证 legado Windows 移植的词典查询链路。</p>
                  </div>
                </body></html>
                """.trimIndent()
            }
            ...
            "/mock/video/search" -> """
                <html><body>
                  <div class="result">
                    <a class="name" href="/mock/video/book">测试剧集</a>
                    <span class="author">出品方</span>
                    <span class="kind">动画</span>
                  </div>
                </body></html>
            """.trimIndent()
            ...
            "/mock/video/book" -> """
                <html><body>
                  <h1 class="bookname">测试剧集</h1>
                  <span class="bookauthor">出品方</span>
                  <div class="intro">用于验证 legado Windows 移植的视频播放链路。</div>
                  <a class="tocurl" href="/mock/video/toc">目录</a>
                </body></html>
            """.trimIndent()

            "/mock/video/toc" -> """
                <html><body><div id="toc">
                  <a class="chapter" href="/mock/video/page?c=0">第1集 起始</a>
                  <a class="chapter" href="/mock/video/page?c=1">第2集 交锋</a>
                  <a class="chapter" href="/mock/video/page?c=2">第3集 决战</a>
                </div></body></html>
            """.trimIndent()

            // 章节页里放 video 标签——验证"从 HTML 里抽播放地址"这条兜底逻辑
            "/mock/video/page" -> {
                val c = q["c"]?.toIntOrNull() ?: 0
                """
                <html><body>
                  <div id="content">
                    <video controls src="/mock/video/file?c=$c"></video>
                  </div>
                </body></html>
                """.trimIndent()
            }
            ...
            "/mock/audio/search" -> """
                <html><body>
                  <div class="result">
                    <a class="name" href="/mock/audio/book">测试有声书</a>
                    <span class="author">播音员</span>
                    <span class="kind">有声小说</span>
                  </div>
                </body></html>
            """.trimIndent()
            ...
            "/mock/audio/book" -> """
                <html><body>
                  <h1 class="bookname">测试有声书</h1>
                  <span class="bookauthor">播音员</span>
                  <div class="intro">用于验证 legado Windows 移植的音频书源链路。</div>
                  <a class="tocurl" href="/mock/audio/toc">目录</a>
                </body></html>
            """.trimIndent()

            "/mock/audio/toc" -> """
                <html><body><div id="toc">
                  <a class="chapter" href="/mock/audio/page?c=0">第1集 起始</a>
                  <a class="chapter" href="/mock/audio/page?c=1">第2集 交锋</a>
                  <a class="chapter" href="/mock/audio/page?c=2">第3集 决战</a>
                </div></body></html>
            """.trimIndent()

            // 章节页里放 audio 标签——验证"从 HTML 里抽播放地址"这条分支
            "/mock/audio/page" -> {
                val c = q["c"]?.toIntOrNull() ?: 0
                """
                <html><body>
                  <div id="content">
                    <audio controls src="/mock/audio/file?c=$c"></audio>
                  </div>
                </body></html>
                """.trimIndent()
            }
            ...
            "/mock/audio/file" -> {
                // 真正返回一段可播放的 WAV（每集音高不同，方便确认换集生效）。
                // 浏览器的媒体栈靠 Range 请求定位/起播，所以这里要支持 Range，
                // 否则 <audio> 会停在 NETWORK_NO_SOURCE 不播放。
                val c = q["c"]?.toIntOrNull() ?: 0
                val bytes = mockWav(440.0 * (1 + c * 0.25))
                serveBytesWithRange(ex, bytes, "audio/wav")
                return
            }

            else -> "<html><body>mock: unknown path</body></html>"
        }
    ...
    /** 生成与内置 mock 站点配套的自测书源 */
    private fun mockBookSourceJson(): String {
        val base = "http://127.0.0.1:$port/mock"
        val src = """
        {
          "bookSourceUrl": "$base",
          "bookSourceName": "本地自测书源",
          "bookSourceGroup": "自测",
          "enabled": true,
          "searchUrl": "$base/search?key={{key}}",
          "ruleSearch": {
